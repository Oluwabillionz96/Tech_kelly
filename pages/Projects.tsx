...
import { Filter, Play, ArrowUpRight } from 'lucide-react';
...
const Projects: React.FC = () => {
  ...
  const categories: (ProjectCategory | 'All')[] = ['All', 'Video Editing', 'Motion Graphics', 'Script Writing', 'Creative Direction'];

  return (
    <div className="pt-32 pb-20 min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        <header className="mb-20">
          <h1 className="text-6xl font-bold font-display mb-8 tracking-tighter">The Vault</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project) => (
            <div key={project.id} className="group bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden hover:border-emerald-500/30 transition-all flex flex-col">
              ...
              <div className="p-8 flex-1 flex flex-col">
                ...
                <div className="flex flex-wrap gap-2 pt-6 border-t border-zinc-800">
                  {project.tags.map(tag => (
                    <span key={tag} className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">{tag}</span>
                  ))}
                </div>
              </div>
            </div>
